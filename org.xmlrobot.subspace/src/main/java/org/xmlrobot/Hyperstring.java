...
public abstract class Hyperstring
	 ...
	 */
	private transient volatile Mass<Character,Integer> mass;
	...
	@Override
	public Mass<Character,Integer> replicator() {
	...
	void echo(TimeListener<?,?> listener, Takion<X,Y> event) {
		...
		if(event.getSource() instanceof Hyperstring) {
			try {
				// listen time synchronously
				Thread.sleep(((Hyperstring)event.getSource()).get().getValue());
			}
			catch(Exception ex) {
				// hum, we must see...
				push(Command.INTERRUPTED);
	...
	protected class Hypermatrix 
		 ...
		 */
		@Override
		public int reproduce(Mass<Character,Integer> root, Mass<Integer,Character> stem) {
			...
			if(cmp > 0) {
				...
				Mass<Character,Integer> pair = instance(
