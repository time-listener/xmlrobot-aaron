...
public abstract class Logic
	 ...
	 */
    public Logic(Class<? extends K> type, Parity gen) {
		super(type, gen);
	}
	/**
	 * {@link Logic} class constructor.
     ...
     * @param gen {@link Parity} the gender
	 */
    public Logic(Class<? extends K> type, V value, Parity gen) {
	 ...
	 * @param gen {@link Parity} the gender
	 */
    public Logic(Class<? extends K> type, Class<? extends V> antitype, Parity gen) {
	 ...
	 */
	@Override
	public Logic<K,V> clone() {
	
		Logic<K,V> logic = (Logic<K,V>) super.clone();
		return logic;
	}
	/* (non-Javadoc)
	...
	public synchronized void swap(K listener) {
		...
		K listenerChild = listener.getChild();
		K listenerParent = listener.getParent();
		...
		// set child as listener's child
		setChild(listenerChild);
		// set child listener's parent as current instance
		listenerChild.setParent(getType().cast(this));

		// set parent as listener's parent
		setParent(listenerParent);
		// set parent listener's child as current instance
		listenerParent.setChild(getType().cast(this));
	}
	/* (non-Javadoc)
